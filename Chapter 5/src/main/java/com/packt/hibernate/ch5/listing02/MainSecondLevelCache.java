package com.packt.hibernate.ch5.listing02;
...
public class MainSecondLevelCache {
	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				Query query = session.createQuery("from Person where id < 10");
...
//				// log stack trace
			...
			// randomly evict some entities from cache.
//			for (int i=0; i < 50; i++) {
//				HibernateUtil.getSessionFactory().getCache().evictEntity(Person.class, randomNumberBetween(0, 50)); //evict a particular Cat
//			}
			...
			try {				
				...
				while (i < 50) {
					i++;
					Person newPerson = (Person) session.get(Person.class, new Long(randomNumberBetween(0, 40)));
